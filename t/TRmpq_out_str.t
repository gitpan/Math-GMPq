...
for(1..5) {
   $ret = TRmpq_out_str($prefix, \*WR4, 10, $mpq, $suffix);
...
for(1..5) {
   $ret = TRmpq_out_str($prefix, \*WR5, 10, $mpq);
...
for(1..5) {
   $ret = TRmpq_out_str($prefix, \*WR6, 10, $mpq, $suffix);
...
while(<RD2>) {
     $count = $.;
     chomp;
     unless($_ eq 'This is the prefix 17'x5) {$ok = 0}
}
...
while(<RD3>) {
     $count = $.;
     chomp;
     unless($_ eq '17 and this is the suffix') {$ok = 0}
}
...
while(<RD4>) {
     $count = $.;
     chomp;
     unless($_ eq 'This is the prefix 17 and this is the suffix') {$ok = 0}
}
...
while(<RD5>) {
     ...
     if($. == 1) {
       ...
       unless($_ eq '17This is the prefix ') {$ok = 0}
     }
}
...
$ret = TRmpq_inp_str($mpq, \*RD8, 10);
close RD8 or die "Can't close RD8: $!";

if($ret == 5 && $mpq == 12351) {print "ok 8\n"}
else {print "not ok 8 $ret $mpq\n"}
